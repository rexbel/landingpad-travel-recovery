...
`HeroLanding`'s frog now shares `FrogSignal`'s construction: its head is the same twin eye-bump silhouette (scaled up) with a matching pair of eyes, so the mid-leap hero and the status icon read as one character rather than two unrelated ones. The 13-name `ProductIcon` family stays pure silhouette (re-colored via the token aliases described above, structurally unchanged) — a small product glyph isn't a context a face reads well in at that size. `FrogSignal` still covers every place the frog is standing still and signaling a state; `HeroLanding` is the one mid-motion appearance. Giving individual `ProductIcon`s a face too would be a separate, larger follow-up if wanted.
